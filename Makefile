...
clean-build: ## remove build artifacts
	rm -fr build/
	rm -fr dist/
	rm -fr roll_regression/rust/target
	rm -fr .eggs/
	rm -f *.so *.dylib *.dll 
...
lint: venv ## check style with flake8
	venv/bin/python -m flake8 roll_regression tests
...
coverage: venv ## check code coverage quickly with the default Python
	venv/bin/python -m coverage run --source roll_regression -m pytest
...
docs: venv ## generate Sphinx HTML documentation, including API docs
	rm -f docs/roll_regression.rst
	rm -f docs/modules.rst
	venv/bin/python -m sphinx-apidoc -o docs/ roll_regression
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
